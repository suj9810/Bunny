...
public class OwnerStoreController {
    ...
    // 가게 수정
    @PutMapping("/{storeId}")
    public ResponseEntity<CommonResponse<Void>> updateStore(@PathVariable Long storeId, @RequestBody @Valid StoreRequestDto requestDto) {
        try {
    ...
    // 전체 가게 조회
    @GetMapping
    public ResponseEntity<CommonResponse<List<StoreResponseDto>>> getAllStores(@RequestParam(required = false) String categories, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {
    ...
    public ResponseEntity<CommonResponse<Void>> closeStore(@PathVariable Long storeId, @RequestBody @Valid StoreStatusDto storeStatusDto) {
        try {
